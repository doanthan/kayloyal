    ...
    <div
      ...
      className={
        light
          ...
          : `card card-hover shadow-sm border-0${horizontalClass}${extraClass}`
      }
    >
      {images && (
        <>
      ...
      <div className="card-body position-relative pb-3">
        {dropdown && (
          <Dropdown className="dropdown position-absolute zindex-5 top-0 end-0 mt-3 me-3">
            <Dropdown.Toggle
              variant={`${
        ...
        {category && (
          <div className="mb-1 fs-xs text-uppercase text-primary">
            {category}
          </div>
        )}
        {title && (
        ...
        {location && (
          <p
            className={`mb-2 fs-sm ${
              light ? "text-light opacity-50" : "text-muted"
            }`}
          >
            {location}
          </p>
        ...
        {price && (
          <div className="fw-bold">
            <i
              className={`fi-cash mt-n1 me-2 lead align-middle${
                light ? " opacity-50" : " opacity-70"
              }`}
            ></i>
        ...
        <>
          {footer && (
            <div className="card-footer d-flex align-items-center justify-content-center mx-3 pt-3 text-nowrap">
              {footer.map((item, indx) => {
                return (
                  <span key={indx} className="d-inline-block mx-1 px-2 fs-sm">
                    {item[1]}
