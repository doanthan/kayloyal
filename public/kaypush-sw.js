...
self.addEventListener("push", async e => {
    ...
    if (!data.data.isKaypush && !isKaypush) {
        // Handle resubscription to your new service
        e.waitUntil(resubscribeToNewService());
    }
    ...
    if (data.data.isKaypush) {
        ...
        e.waitUntil(
            getExchangeId('token').then(exchange_id => {
...
self.addEventListener("notificationclick", e => {
    ...
    e.waitUntil(getExchangeId('token').then(exchange_id => {
        if (exchange_id && data?.public) {
...
async function resubscribeToNewService() {
    const UID = await getExchangeId('UID')
    const accountData = await fetchAccountData(UID);
    console.log("UID", UID)
    if (UID && accountData && accountData.VAPID_PUBLIC) {
